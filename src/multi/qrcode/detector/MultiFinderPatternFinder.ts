...
class MultiFinderPatternFinder extends FinderPatternFinder {
  ...
  private selectMutipleBestPatterns(): FinderPattern[][] {
    ...
    for (let i1 = 0; i1 < size - 2; i1++) {
      ...
      for (let i2 = i1 + 1; i2 < size - 1; i2++) {
        ...
        const vModSize12 =
          (p1.estimatedModuleSize - p2.estimatedModuleSize) /
          Math.min(p1.estimatedModuleSize, p2.estimatedModuleSize);
        const vModSize12A = Math.abs(
          p1.estimatedModuleSize - p2.estimatedModuleSize
        );

        if (
          vModSize12A > DIFF_MODSIZE_CUTOFF &&
        ...
        for (let i3 = i2 + 1; i3 < size; i3++) {
          ...
          const vModSize23A = Math.abs(
            p2.estimatedModuleSize - p3.estimatedModuleSize
          );

          if (
            vModSize23A > DIFF_MODSIZE_CUTOFF &&
          ...
          const info: FinderPatternFinderResult = new FinderPatternFinderResult(
            test[1],
            test[2],
            test[0]
          );
          const dA = FinderPatternFinder.distance(
            info.topLeft,
            info.bottomLeft
          );
          const dC = FinderPatternFinder.distance(
            info.topRight,
            info.bottomLeft
          );
          ...
          if (
            estimatedModuleCount > MAX_MODULE_COUNT_PER_EDGE ||
  ...
  public findMulti(hints?: Map<DecodeHint, any>): FinderPatternFinderResult[] {
          ...
          if ((currentState & 1) == 0) {
            ...
            if (currentState == 4) {
              ...
              if (
                ...
                this.handlePossibleCenter(stateCount, i, horizontalIndex)
              ) {
                // Yes
    ...
    patternInfo.forEach((pattern: FinderPattern[]) => {
      ...
      result.push(
        new FinderPatternFinderResult(pattern[1], pattern[2], pattern[0])
      );
    });
