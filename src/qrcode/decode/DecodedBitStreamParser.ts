...
export default class DecodedBitStreamParser {
    ...
    do {
      ...
      switch (mode.bits) {
        ...
        default:
          ...
          const count: number = bits.readBits(
            mode.getCharacterCountBits(version)
          );
          switch (mode.bits) {
            case NumericMode.BITS:
              result += DecodedBitStreamParser.decodeNumericSegment(
                bits,
                count
              );
              break;
            case AlphaNumericMode.BITS:
              result += DecodedBitStreamParser.decodeAlphanumericSegment(
                bits,
                count,
                fc1InEffect
              );
              break;
            case ByteMode.BITS:
              result += DecodedBitStreamParser.decodeByteSegment(
                bits,
                count,
                byteSegments
              );
  ...
  private static decodeKanjiSegment(bits: BitSource, count: number): string {
    let result: string = "";
  ...
  private static decodeAlphanumericSegment(
    ...
    while (count > 1) {
      ...
      result += DecodedBitStreamParser.toAlphaNumericChar(
        nextTwoCharsBits / 45
      );
      result += DecodedBitStreamParser.toAlphaNumericChar(
        nextTwoCharsBits % 45
  ...
  private static decodeNumericSegment(bits: BitSource, count: number): string {
    let result: string = "";
