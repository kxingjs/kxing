...
export default class QRDetector {
  ...
  protected processFinderPatternInfo(
    finderPatterns: FinderPatternFinderResult
  ): BitMatrix {
    ...
    if (moduleSize < 1.0) {
      throw new NotFoundError(
        "Couldn't detect a code that has module size enough."
      );
    }

    const dimension: number = QRDetector.computeDimension(
      topLeft,
      topRight,
      bottomLeft,
      moduleSize
    );
    const provisionalVersion: Version = Version.getProvisionalVersionForDimension(
      dimension
    );
    const modulesBetweenFPCenters: number =
    ...
    if (provisionalVersion.alignmentPatternCenters.length > 0) {
      ...
      const estAlignmentX: number = Math.floor(
        topLeft.x + correctionToTopLeft * (bottomRightX - topLeft.x)
      );
      const estAlignmentY: number = Math.floor(
    ...
    const transform: PerspectiveTransform = QRDetector.createTransform(
      topLeft,
      topRight,
      bottomLeft,
      alignmentPattern,
  ...
  private static computeDimension(
    ...
    const tltrCentersDimension = round(
      distance(topLeft, topRight) / moduleSize
    );
    const tlblCentersDimension = round(
  ...
  protected findAlignmentInRegion(
    ...
    const alignmentAreaRightX = Math.min(
      this._width - 1,
    ...
    const alignmentAreaBottomY = Math.min(
      this._height - 1,
