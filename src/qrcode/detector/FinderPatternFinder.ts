...
export default class FinderPatternFinder {
  ...
  public find(hints?: Map<DecodeHint, any>): FinderPatternFinderResult {
    ...
    const bestPatterns: FinderPattern[] = FinderPatternFinder.orderBestPatterns(
      patternInfo
    );

    return new FinderPatternFinderResult(
      bestPatterns[1],
      bestPatterns[2],
      bestPatterns[0]
    );
  ...
  protected handlePossibleCenter(
    ...
    const stateCountTotal: number = stateCount.reduce(function(
      previousValue,
      currentValue
    ) {
      return previousValue + currentValue;
    });
    let centerHorizontalIndex: number =
      horizontalIndex - stateCount[4] - stateCount[3] - stateCount[2] / 2.0;
    let centerVerticalIndex: number = this.crossCheckVertical(
      verticalIndex,
      Math.floor(centerHorizontalIndex),
      stateCount[2],
      stateCountTotal
    );
    ...
    if (!isNaN(centerVerticalIndex)) {
      // Re-cross check
      centerHorizontalIndex = this.crossCheckHorizontal(
        Math.floor(centerHorizontalIndex),
        Math.floor(centerVerticalIndex),
        stateCount[2],
        stateCountTotal
      );
      if (!isNaN(centerHorizontalIndex)) {
        ...
        let found: boolean = this._possibleCenters.some(function(center) {
          return center.aboutEquals(
            estimatedModuleSize,
            centerVerticalIndex,
            centerHorizontalIndex
          );
        });
  ...
  private selectBestPatterns(): FinderPattern[] {
    ...
    if (startSize > 3) {
      ...
      this._possibleCenters.sort(function(
        center1: FinderPattern,
        center2: FinderPattern
      ) {
      ...
      for (
        let i = 0;
        i < this._possibleCenters.length && this._possibleCenters.length > 3;
        i++
      ) {
    ...
    return [
      this._possibleCenters[0],
      this._possibleCenters[1],
      this._possibleCenters[2]
    ];
  }
   ...
   */
  protected static orderBestPatterns(
  ...
  protected static distance(
    pattern1: FinderPattern,
    pattern2: FinderPattern
  ): number {
    return Math.sqrt(
      Math.pow(pattern1.x - pattern2.x, 2) +
        Math.pow(pattern1.y - pattern2.y, 2)
    );
  }
}
