...
export default class GenericGFPoly {
    ...
    evaluateAt(a: number): number {
        ...
        for (let i = 1; i < size; i++) {
            result = GenericGF.addOrSubtract(this._field.multiply(a, result), this._coefficients[i]);
        }
        return result;
    }
    ...
    addOrSubtract(other: GenericGFPoly): GenericGFPoly {
        if (this._field != other.field) {
            throw new IllegalArgumentError("GenericGFPolys do not have same GenericGF field");
        }
        if (this.isZero()) {
            return other;
        }
    ...
    multiply(other: GenericGFPoly): GenericGFPoly {
        ...
        for (let i = 0; i < aLength; i++) {
            ...
            for (let j = 0; j < bLength; j++) {
                product[i + j] = GenericGF.addOrSubtract(product[i + j], this._field.multiply(aCoeff, bCoefficients[j]));
            }
        }
    ...
    divide(other: GenericGFPoly): GenericGFPoly[] {
        ...
        const inverseDenominatorLeadingTerm: number = this._field.inverse(denominatorLeadingTerm);

        while (remainder.degree >= other.degree && !remainder.isZero()) {
            const degreeDifference = remainder.degree - other.degree;
            const scale: number = this._field.multiply(remainder.getCoefficient(remainder.degree), inverseDenominatorLeadingTerm);
            const term: GenericGFPoly = other.multiplyByMonomial(degreeDifference, scale);
            const iterationQuotient: GenericGFPoly = this._field.buildMonomial(degreeDifference, scale);
