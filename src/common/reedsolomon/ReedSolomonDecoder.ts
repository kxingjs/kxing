...
export default class ReedSolomonDecoder {
    ...
    decode(received: number[], twoS: number): void {
        ...
        for (let i = 0; i < twoS; i++) {
            const evaluate = poly.evaluateAt(this._field.exp(i + this._field.generatorBase));
        ...
        const syndrome: GenericGFPoly = new GenericGFPoly(this._field, syndromeCoefficients);
        const sigmaOmega: GenericGFPoly[] = this.runEuclideanAlgorithm(this._field.buildMonomial(twoS, 1), syndrome, twoS);
    ...
    private runEuclideanAlgorithm(a: GenericGFPoly, b: GenericGFPoly, R: number): GenericGFPoly[] {
        ...
        while (r.degree >= R / 2) {
            ...
            while (r.degree >= rLast.degree && !r.isZero()) {
                ...
                const scale: number = this._field.multiply(r.getCoefficient(r.degree), dltInverse);
    ...
    private findErrorMagnitudes(errorEvaluator: GenericGFPoly, errorLocations: number[]) {
        ...
        for (let i = 0; i < s; i++) {
            ...
            for (let j = 0; j < s; j++) {
                if (i != j) {
                    const term: number = this._field.multiply(errorLocations[j], xiInverse);
